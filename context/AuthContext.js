...
export const useAuth = create((set) => ({
  ...
  signup: async (email, password, name) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axios.post(`${API_URL}/signup/phone`, { email, password, name });
      await AsyncStorage.setItem("token", response.data.token);
      set({ user: response.data.user, isAuthenticated: true, isLoading: false });
    } catch (error) {
      set({ error: error.response?.data?.message || "Error signing up", isLoading: false });
  ...
  login: async (email, password) => {
    set({ isLoading: true, error: null });
    try {
      const response = await axios.post(`${API_URL}/login/phone`, { email, password });
      await AsyncStorage.setItem("token", response.data.token);
      set({ isAuthenticated: true, user: response.data.user, error: null, isLoading: false });
    } catch (error) {
      set({ error: error.response?.data?.message || "Error logging in", isLoading: false });
  ...
  checkAuth: async () => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      if (!token) throw new Error("No token found");
      const response = await axios.get(`${API_URL}/check-auth/phone`, { headers: { Authorization: `Bearer ${token}` } });
      set({ user: response.data.user, isAuthenticated: true, isCheckingAuth: false });
    } catch (error) {
      await AsyncStorage.removeItem("token");
  ...
  updateProfile: async (userID, name, country, color, userImage, supplies) => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      const response = await axios.post(`${API_URL}/update-profile`, { userID, name, country, color, userImage, supplies }, { headers: { Authorization: `Bearer ${token}` } });
      set({ message: response.data.message, isLoading: false });
    } catch (error) {
      set({ isLoading: false, error: error.response?.data?.message || "Error updating profile" });
      throw error;
  ...
  addProfileImg: async (userID, imgFile) => {
    ...
    formData.append("profile-image", {
      uri: imgFile.uri,
      name: imgFile.fileName || "profile.jpg",
  ...
  delProfileImg: async (userID) => {
    ...
    try {
      const token = await AsyncStorage.getItem("token");
      const response = await axios.post(`${API_URL}/del-profile-img`, { userID }, { headers: { Authorization: `Bearer ${token}` } });
      set({ message: response.data.message, isLoading: false });
    } catch (error) {
      set({ isLoading: false, error: error.response?.data?.message || "Error deleting image" });
      throw error;
